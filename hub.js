...
// Hub.js 0.0.2
// (c) 2012 Christopher de Beer.
// All rights reserved. & Not Available under any licence (yet)
// You may only use the source for reference. Till I pick a licence to release it under.
...
var Hub = function(){
	...
	var Hub = {
		...
		addRequirements: function(item, requires) {
			...
			if (__.isUndefined(items[item])) Hub.newItem(item, null);
			if (!__.isUndefined(items[item])) {				
				if (requires instanceof Array) {

					for (x in requires) {
						if (debug) console.log("looping through requires:", requires[x])
						if (requires[x] !== "" && !__.isUndefined(requires[x]) && requires[x] !== null) {
							if (__.isUndefined(items[item].requires[requires[x]])) items[item].requires[requires[x]] = true;
							if (__.isUndefined(items[requires[x]])) {
								Hub.newItem(requires[x], null);
							}
							items[requires[x]].dependants = __.union(items[requires[x]].dependants,[item]);
						} else {
		...
		get: function (item, callback) {
			...
			if (debug) console.log("Attempting to Get item ["+item+"]")

			// if there's no such item, then error
			if (__.isUndefined(items[item])) callback(new Error("No Such item ["+item+"]"));

			// if the item is null and has no getter, then error
			else if (__.isNull(items[item].value) && __.isUndefined(items[item].get)) callback(new Error("No Getter is set for that item ["+item+"], and Value is <null>."));
			
			// if the item has a value and a getter 
			else if (!__.isNull(items[item].value) && !__.isUndefined(items[item].get)) {
				...
				if (items[item].value instanceof Promise) {
					...
					if (!__.isUndefined(callback)) items[item].callbacks.push(callback);
					else if (debug) console.log("No callback to attach to promise.")
				
				// if the value is not a promise
				} 
				//else {					

				// if this value has requirements that are not met, then ask & wait
				var req = Hub.unmetRequires(item);
				if (req.length > 0) {

					Hub.promise(item);
					if (debug) console.log("There are unmet dependancies for ["+item+"] ie: outstanding promises, waiting for them: ", req);
					if (!__.isUndefined(callback)) items[item].callbacks.push(callback);
					// items[item].get();

				// this value has no unmet dependancies. then send
				} else {
					if (!__.isUndefined(callback)) callback(items[item].value);
					if (!__.isUndefined(items[item].get)) items[item].get()
				}
		...
		isPromise: function(item) {
			...
			if (__.isUndefined(items[item])) throw new Error("No such item ["+item+"] exists.");
			else if (__.isNull(items[item].value)) return true;
		...
		set: function(item, value) {
			...
			if (__.isUndefined(items[item])) {
				...
				if (!__.isEqual(items[item].value, value)) {
					items[item].value = value;

					// calling this items setter if it exists
					if (!__.isUndefined(items[item].set)) items[item].set();
					...
					if (items[item].callbacks.length > 0) {
						// if (debug) console.log("Callbacks: ", items[item].callbacks);
		...
		middleware: function(req, res, next) {
			if (req && req.url === "/hub/client.js") {
				...
				// make sure the path below links to THIS file
				// wired inception where this file serves a copy of itself to clients
				// via attaching to connect/express middleware

				fs.readFile('../hub.js', function(error, content) {
			        if (error) {
			            res.writeHead(500);
			            res.end("error: " + error);
			        } else {
			            res.writeHead(200, {'Content-Type': 'text/plain'});
						res.end(";var clientHub = " + JSON.stringify(Hub.filterEntitiesByClient(req), function(key, val) {
		...
		filterEntitiesByClient: function (req) {
			var entities = {};
			__.each(items, function(i) {
				var scopeCheck = i.name.substring(0,("client." + config.clients.clientIdentifier(req) + "." + config.clients.scope).length)
				var scopeMatch = scopeCheck === "client." + config.clients.clientIdentifier(req) + "." + config.clients.scope;
				// if (debug) console.log("Filtering entities by client: ", i.name, scopeCheck, scopeMatch)
				if (scopeMatch) {
					i.name = i.name.replace("client." + config.clients.clientIdentifier(req) + ".", "")
					// i.set = config.clients.defaultSetter;
					entities[i.name] = i;
				}
			})
			return entities;
		},
		setupClients: function(options) {
			var defaults = {
				server: undefined,
				scope: "ux",
				clientIdentifier: function(req) {
					return "anon";
				}
			}
			config.clients = __.defaults(options, defaults);

			// set up dnode and make it listen to the same server
			// put all functions that the client needs to be able to accesss here

			server = dnode({
				set: function(n, cb) {
					cb(null, "Hi im speacking to you from beyond the network. You said: " + n);
				}
			});
			server.listen(config.clients.server);

			
		},
		setClientEntity: function (req, item, value) {
			Hub.set("client." + config.clients.clientIdentifier(req) + "." + item, value)
		},
		setServerEntity: function (item, value, callback) {

			DNode.connect(function (remote) {
                remote.set({entity: item, value: value}, callback);
            });
		}
	};
